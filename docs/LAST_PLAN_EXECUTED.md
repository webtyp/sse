...
# Plan — `webtyp.com/sse`: a list stops claiming it has columns
...
**Files:** `models_orm.go` (1 list type).
...
3. Confirm the diff contains **only** removals of the two stub methods —
   1 `Schema()` and 1 `Pointers()` lines — and nothing else. If
...
| Stage | Files | Action |
|---|---|---|
| 1 | `models_orm.go` | regenerate with `ormc`; 1 stub pair disappear |
